...
if __name__ == '__main__':
    ...
    # note: below is an example of running a Spark process from the command
    # line (using a .py file)
    ...
    resp = requests.get(
        "https://hacker-news.firebaseio.com/v0/item/16222426.json")
    ...
    ######################################################################
    # Setting up Airflow:
    # note: below is an example of setting up Airflow by putting the DAG .py
    # file in the correct folder for Airflow to use

    # repl:~$ echo $AIRFLOW_HOME
    # ~/airflow
    # repl:~$ ls ~/airflow
    # airflow.cfg      airflow.db          airflow-webserver.pid  logs
    # airflow.cfg.bak  airflow.db-journal  dags                   unittests.cfg
    ...
    connection_uri = (
        "postgresql://repl:password@localhost:5432/datacamp_application")
    db_engine = sqlalchemy.create_engine(connection_uri)
    ...
    user2 = pd.read_sql("SELECT * FROM rating WHERE user_id = 18163",
                        db_engine)
    ...
    def transform_recommendations(avg_course_ratings, courses_to_recommend):
        ...
        grouped = merged.sort_values("rating", ascending=False).groupby(
            'user_id')
    ...
    def recommendations_for_user(user_id, threshold=4.5):
      ...
      """
        ...
        predictions_df = pd.read_sql(query, db_engine,
